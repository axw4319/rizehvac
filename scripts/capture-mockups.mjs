...
async function run() {
  ...
  for (const vp of VIEWPORTS) {
    ...
    try {
      await indexPage.goto(`${BASE}/preview`, { waitUntil: "networkidle", timeout: 45000 });
      await indexPage.waitForTimeout(800);
      await indexPage.screenshot({
        path: `${OUT}/${vp.name}-index.png`,
        fullPage: true,
      });
      count++;
      console.log(`✓ ${OUT}/${vp.name}-index.png`);
    } catch (e) {
      console.log(`✗ index ${vp.name} :: ${e.message}`);
    }
    await indexPage.close();
    await ctx.close();
  }
  await browser.close();
  console.log(`\nDone. ${count} screenshots in ${((Date.now() - start) / 1000).toFixed(1)}s.`);
}
